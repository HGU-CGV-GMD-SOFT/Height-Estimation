...
class ImageEditor:
    ...
    def cal_height(self):
        ...
        print(rb)
        b_line = (rb,mb)
        cv2.line(self.image, rb, mb, (255, 255, 0), 2)
        ...
        cv2.line(self.image, vertex, mb, (255, 255, 0), 2)
        ...
        cv2.line(self.image, vertex, t, (0, 0, 0), 2)
    ...
    def draw_marker(self, intersection_point, color):
        ...
        if intersection_point:
            cv2.circle(self.image, intersection_point, 5, color, -1)
            self.update_displayed_image()
    ...
    def display_mini_image(self):
        # Calculate the region to display in the mini image
        top_left_x = max(0, self.mouse_x - 50)
        top_left_y = max(0, self.mouse_y - 50)
        bottom_right_x = min(self.image.shape[1], self.mouse_x + 50)
        bottom_right_y = min(self.image.shape[0], self.mouse_y + 50)

        # Adjust the offsets if the calculated region exceeds the bounds of the main image
        if top_left_x == 0:
            bottom_right_x += 50 - self.mouse_x  # Adjust right edge
        if top_left_y == 0:
            bottom_right_y += 50 - self.mouse_y  # Adjust bottom edge
        if bottom_right_x == self.image.shape[1]:
            top_left_x -= 50 - (self.image.shape[1] - self.mouse_x)  # Adjust left edge
        if bottom_right_y == self.image.shape[0]:
            top_left_y -= 50 - (self.image.shape[0] - self.mouse_y)  # Adjust top edge

        # Extract the region from the main image
        image_zoom = self.image[top_left_y:bottom_right_y, top_left_x:bottom_right_x]

        # Resize the region to fit the mini image label
        ...
        # Update the mini image label with the new image
        self.mini_image_label.config(image=mini_photo)
        self.mini_image_label.image = mini_photo  # Keep a reference to prevent garbage collection
